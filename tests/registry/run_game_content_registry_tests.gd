...
func _initialize() -> void:
	...
	if registry.is_valid():
		_expect(registry.factions.size() == 8, "Expected 8 factions.")
		_expect(registry.npcs.size() == 12, "Expected 12 handcrafted NPCs.")
		_expect(registry.voices.size() == 13, "Expected 13 voice profiles.")
		_expect(registry.ships.size() == 13, "Expected 13 ship designs.")
		_expect(registry.portraits.size() == 162, "Expected 162 portraits.")
