...
func on_system_entered(system_id: String, _arrival_gate_id: String) -> void:
	...
	if neighbor_defs.is_empty():
		return

	var jobs: Array[Dictionary] = []
	for dest_def: SystemDefinition in neighbor_defs:
		var config: SystemConfig = _system_registry.get_generated_config(str(dest_def.id))
		if config == null:
			config = _system_registry.get_generated_config(dest_def.legacy_id)
		if config == null:
			continue
		for i in range(config.npc_patrol_count):
			var seed_str: String = "ship_%d_%d" % [config.seed_value, i + 1]
			var cached_path: String = "res://assets/ships/generated/%s.glb" % seed_str
