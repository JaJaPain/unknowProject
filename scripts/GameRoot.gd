...
func request_chapter_plan_generation() -> Dictionary:
	if campaign_chapter_packet_store == null:
		_initialize_campaign_chronicle()
	if campaign_chapter_packet_store == null \
			or not campaign_chapter_packet_store.is_valid():
	...
	LLMInterface.request_chapter_plan_generation(
		...
		_chapter_plan_unresolved_story_state(),
		objective_types,
		valid_entity_ids,
		_on_chapter_plan_generation_result
	)
...
func _request_chapter_plan_generation_for_active_slot() -> void:
	var requested := request_chapter_plan_generation()
	if not bool(requested.get("ok", false)):
		push_warning(
...
func _on_chapter_plan_generation_result(result: Dictionary) -> void:
	...
	if campaign_chapter_packet_store == null \
		...
		return
	if bool(result.get("ok", false)):
		var committed := campaign_chapter_packet_store.append_packet(result.get("packet", {}))
		if bool(committed.get("ok", false)):
...
func _chapter_plan_unresolved_story_state() -> Dictionary:
