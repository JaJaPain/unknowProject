...
def get_aspect_ratio(face):
    if not face.is_valid:
        return 1.0
    length1 = face.edges[0].calc_length()
    length2 = face.edges[1].calc_length()
    if length2 == 0:
        return 1000.0 # Return a very high aspect ratio to effectively fail the ratio check
    face_aspect_ratio = max(0.01, length1 / length2)
    ...
    for face in result['geom']:
        if isinstance(face, bmesh.types.BMFace):
            if is_rear_face(face):
                ...
                global engine_coords
                engine_coords.append(face.calc_center_bounds().copy())
...
def add_weapons_to_face(bm, face):
    ...
    for h in range(horizontal_step):
        ...
        for v in range(vertical_step):
            ...
            global weapon_coords
            weapon_coords.append((pos + face.normal * weapon_depth * 2).copy())
...
def add_hull_normal_map(mat, hull_normal_map):
    ...
    teximage_node.projection ='FLAT'
    tex_coords_node = ntree.nodes.new('ShaderNodeTexCoord')
    links.new(tex_coords_node.outputs['UV'], teximage_node.inputs['Vector'])
...
def set_hull_mat_basics(mat, color, hull_normal_map, texture_file='metal.png'):
    shader_node = getShaderNode(mat)
    shader_node.inputs["Specular IOR Level"].default_value = 0.5
    shader_node.inputs["Metallic"].default_value = 0.85
    shader_node.inputs["Roughness"].default_value = 0.35
    
def set_hull_mat_basics(mat, texture_file, normal_file, is_dark=False, metallic=0.85):
    ntree = mat.node_tree
    shader_node = getShaderNode(mat)
    
    is_atlas = False
    atlas_uv_bounds = None
    actual_texture_file = texture_file
    
    metadata_path = resource_path('textures\\metals', 'ShipPaint_Textures.json')
    if os.path.exists(metadata_path):
        with open(metadata_path, 'r') as f:
            metadata = json.load(f)
            for t in metadata.get('textures', []):
                if t['name'] == texture_file:
                    is_atlas = True
                    actual_texture_file = metadata.get('image', 'ShipPaint.png')
                    atlas_rect = t
                    break

    # Metal Texture
    try:
        metal_img = bpy.data.images.load(resource_path('textures\\metals', actual_texture_file), check_existing=True)
        if is_atlas and metal_img.size[0] > 0:
            import numpy as np
            img_width, img_height = metal_img.size
            src_pixels = np.zeros(img_width * img_height * 4, dtype=np.float32)
            metal_img.pixels.foreach_get(src_pixels)
            src_pixels = src_pixels.reshape((img_height, img_width, 4))
            ...
            y_start = img_height - y - height
            y_end = y_start + height
            x_start = x
            x_end = x + width
            
            cropped_pixels = src_pixels[y_start:y_end, x_start:x_end]
            
            metal_img = bpy.data.images.new(texture_file, width, height, alpha=True)
            metal_img.pixels.foreach_set(cropped_pixels.flatten())
            metal_img.pack()
    except Exception as e:
        print(f"Failed to load metal texture: {e}")
        metal_img = bpy.data.images.new("Fallback", 1, 1)

    teximage_node = ntree.nodes.new('ShaderNodeTexImage')
    teximage_node.image = metal_img
    
    # Normal Map
    try:
        hull_normal_map = bpy.data.images.load(resource_path('textures\\normals', normal_file), check_existing=True)
    except Exception as e:
        print(f"Failed to load normal map: {e}")
        hull_normal_map = bpy.data.images.new("FallbackNormal", 1, 1)
        
    tex_coords_node = add_hull_normal_map(mat, hull_normal_map)
    
    # Add Mapping node to stretch the metal texture (prevents 90s repeating pattern)
    mapping_node = ntree.nodes.new('ShaderNodeMapping')
    mapping_node.inputs['Scale'].default_value = (0.2, 0.2, 0.2)
    ntree.links.new(tex_coords_node.outputs['UV'], mapping_node.inputs['Vector'])
    
    # Base Color connection
    ntree.links.new(mapping_node.outputs['Vector'], teximage_node.inputs['Vector'])
    
    if is_dark:
        darken_node = ntree.nodes.new('ShaderNodeMixRGB')
        darken_node.blend_type = 'MULTIPLY'
        darken_node.inputs[0].default_value = 1.0
        darken_node.inputs[2].default_value = (0.6, 0.6, 0.6, 1.0)
        ntree.links.new(teximage_node.outputs['Color'], darken_node.inputs[1])
        ntree.links.new(darken_node.outputs[0], shader_node.inputs['Base Color'])
    else:
        ntree.links.new(teximage_node.outputs['Color'], shader_node.inputs['Base Color'])
    
    # Other BSDF settings
    shader_node.inputs["Specular IOR Level"].default_value = 0.5
    shader_node.inputs["Metallic"].default_value = metallic
    
    # As metallic goes down, it becomes more rough. If metallic is 0, roughness is ~0.8. If metallic is 1, roughness is ~0.35
    roughness = 0.8 - (metallic * 0.45)
...
def get_badge_info(emblem_id):
    metadata_path = resource_path('textures\\badges', 'badge_sheets_metadata.json')
    try:
        if os.path.exists(metadata_path):
            with open(metadata_path, 'r') as f:
                metadata = json.load(f)
                for sheet in metadata.get('sheets', []):
                    for sprite in sheet.get('sprites', []):
                        if sprite['id'] == emblem_id:
                            rect = sprite.get('trimmedRect', sprite.get('rect'))
                            width = sheet['imageSize']['width']
                            height = sheet['imageSize']['height']
                            uv_min_x = rect['x'] / width
                            uv_max_x = (rect['x'] + rect['width']) / width
                            uv_max_y = 1.0 - (rect['y'] / height)
                            uv_min_y = 1.0 - ((rect['y'] + rect['height']) / height)
                            return sheet['file'], (uv_min_x, uv_min_y, uv_max_x, uv_max_y)
    except Exception as e:
        print(f"Failed to read badge metadata: {e}")
    return emblem_id, None

# Creates the materials
def create_materials(texture_file='metal.png', emblem_file='none', normal_file='hull_normal.png', metallic=0.85):
    ret = []
    for mat in Material:
        bpy_mat = bpy.data.materials.new(mat.name)
        bpy_mat.use_nodes = True
        ret.append(bpy_mat)

    # Build the hull texture
    mat = ret[Material.hull]
    set_hull_mat_basics(mat, texture_file, normal_file, metallic=metallic)

    # Build the hull_lights texture
    mat = ret[Material.hull_lights]
    tex_coords_node = set_hull_mat_basics(mat, texture_file, normal_file, metallic=metallic)
    ntree = mat.node_tree
    shader_node = getShaderNode(mat)
    links = ntree.links

    # We do NOT mix the diffuse map anymore because it was causing the solid white panels.
    # The TintNode from set_hull_mat_basics is already securely plugged into the Base Color.
    ...
    teximage_emit_node.projection ='FLAT'
    links.new(tex_coords_node.outputs['UV'], teximage_emit_node.inputs['Vector'])
    ...
    set_hull_mat_basics(mat, texture_file, normal_file, is_dark=True, metallic=metallic)
    ...
    # Build the decal material
    mat = ret[Material.decal]
    mat.blend_method = 'HASHED'
    if emblem_file and emblem_file.lower() != 'none':
        ntree = mat.node_tree
        shader_node = getShaderNode(mat)
        
        actual_file, _ = get_badge_info(emblem_file)
        try:
            emblem_img = bpy.data.images.load(resource_path('textures\\badges', actual_file), check_existing=True)
            teximage_node = ntree.nodes.new('ShaderNodeTexImage')
            teximage_node.image = emblem_img
            
            ntree.links.new(teximage_node.outputs['Color'], shader_node.inputs['Base Color'])
            ntree.links.new(teximage_node.outputs['Alpha'], shader_node.inputs['Alpha'])
        except Exception as e:
            print(f"Failed to load emblem: {e}")

    return ret

def add_decals(bm, ship_class='hauler', emblem_file='none'):
    ...
    if ship_class == 'fighter':
        ...
        for face in bm.faces[:]:
            # Look for central faces, reasonably sized, pointing up or down
            if get_aspect_ratio(face) < 3.0 and face.calc_area() > 0.02 and abs(face.calc_center_bounds().y) < 0.3:
                if face.normal.z > 0.9:
                    top_candidates.append(face)
                elif face.normal.z < -0.9:
                    bottom_candidates.append(face)
                    
        # Sort by highest X (furthest forward)
        top_candidates.sort(key=lambda f: f.calc_center_bounds().x, reverse=True)
        bottom_candidates.sort(key=lambda f: f.calc_center_bounds().x, reverse=True)
        
        chosen_faces = []
        if top_candidates:
            chosen_faces.append(top_candidates[0])
        elif bottom_candidates:
            chosen_faces.append(bottom_candidates[0])
    else:
        for face in bm.faces[:]:
            if abs(face.normal.y) > 0.9 and get_aspect_ratio(face) < 2.0 and face.calc_area() > 0.1:
                decal_candidates.append(face)
            
        if decal_candidates:
            import random as rand
            rand.shuffle(decal_candidates)
            chosen_faces = decal_candidates[:2]
        else:
            chosen_faces = []
    ...
    for face in chosen_faces:
            ...
            if normal.z > 0.9:
                ref_vec = Vector((-1, 0, 0)) # Flipped X-axis to rotate badge 180 degrees
            elif normal.z < -0.9:
                ref_vec = Vector((1, 0, 0)) # Flipped for the bottom so it reads correctly from below
            else:
                ref_vec = Vector((0, 0, 1)) # Use Z-axis as reference
                
            right_vec = normal.cross(ref_vec).normalized()
            up_vec = right_vec.cross(normal).normalized()
            
            size = min(face.calc_area() ** 0.5 * 0.4, 0.8)
            
            p1 = center - right_vec * size - up_vec * size * 2.0
            p2 = center + right_vec * size - up_vec * size * 2.0
            p3 = center + right_vec * size + up_vec * size * 2.0
            p4 = center - right_vec * size + up_vec * size * 2.0
            ...
            new_face = bm.faces.new((v1, v2, v3, v4))
            new_face.material_index = Material.decal
            
            # Flipped horizontally to prevent mirrored text!
            actual_file, uv_bounds = get_badge_info(emblem_file)
            if uv_bounds:
                uv_min_x, uv_min_y, uv_max_x, uv_max_y = uv_bounds
                uvs = [
                    (uv_max_x, uv_min_y),
                    (uv_min_x, uv_min_y),
                    (uv_min_x, uv_max_y),
...
# Generates a complete spaceship mesh and material, returning the blender object.
def generate_spaceship(random_seed='', ship_class='hauler', num_hull_segments_min=3, num_hull_segments_max=6, num_asymmetry_segments_min=1, num_asymmetry_segments_max=5, texture_file='metal.png', emblem_file='none', normal_file='hull_normal.png', metallic=0.85, apply_bevel_modifier=True, assign_materials=True):
    
    if random_seed:
        seed(random_seed)
        
    global engine_coords, weapon_coords
    engine_coords = []
    weapon_coords = []
    ...
    if ship_class == 'fighter':
        scale_vector = Vector((uniform(1.5, 3.0), uniform(1.2, 2.5), uniform(0.2, 0.5)))
    else:
            ...
            for i in hull_segment_range:
                ...
                if val > 0.1:
                    ...
                    if random() > 0.5:
                        ...
                        if ship_class == 'fighter' and face.normal.x > 0.5:
                            # Force aggressive taper for the nose
                            sy = uniform(0.5, 0.8)
                            sz = uniform(0.5, 0.8)
    ...
    for face in bm.faces[:]:
        ...
        if random() > 0.85:
            ...
            for i in range(randrange(num_asymmetry_segments_min, num_asymmetry_segments_max)):
                ...
                if ship_class == 'fighter' and abs(face.normal.y) > 0.5:
                    # Sweep wings back
                    bmesh.ops.translate(bm, vec=Vector((-hull_piece_length * uniform(1.0, 2.5), 0, 0)), verts=face.verts)
                    # Taper wings more for sharper edges
                    scale_face(bm, face, uniform(0.5, 0.8), uniform(0.5, 0.8), uniform(0.5, 0.8))
    ...
    if emblem_file and emblem_file.lower() != 'none' and ship_class != 'fighter':
        add_decals(bm, ship_class, emblem_file)

    # Apply horizontal symmetry sometimes
    if random() > 0.5:
        bmesh.ops.symmetrize(bm, input=bm.verts[:] + bm.edges[:] + bm.faces[:], direction="Y")
        
        # Fix mirrored decal UVs caused by the symmetrize operation
        if emblem_file and emblem_file.lower() != 'none':
            _, uv_bounds = get_badge_info(emblem_file)
            uv_layer = bm.loops.layers.uv.verify()
            for face in bm.faces:
                if face.material_index == Material.decal and face.calc_center_bounds().y < 0:
                    for loop in face.loops:
                        if uv_bounds:
                            uv_min_x, _, uv_max_x, _ = uv_bounds
                            loop[uv_layer].uv[0] = uv_max_x + uv_min_x - loop[uv_layer].uv[0]
                        else:
                            loop[uv_layer].uv[0] = 1.0 - loop[uv_layer].uv[0]

    # Add fighter decals AFTER symmetry so they can be placed exactly in the center without being sliced!
    if emblem_file and emblem_file.lower() != 'none' and ship_class == 'fighter':
        add_decals(bm, ship_class, emblem_file)
    ...
    # Add engine and weapon empties
    for i, coord in enumerate(engine_coords):
        empty = bpy.data.objects.new(f"Engine_{i+1}", None)
    ...
    for i, coord in enumerate(weapon_coords):
        empty = bpy.data.objects.new(f"Weapon_{i+1}", None)
        empty.empty_display_type = 'ARROWS'
        empty.location = coord
        empty.parent = obj
        scene.collection.objects.link(empty)
    ...
    me = ob.data
    materials = create_materials(texture_file, emblem_file, normal_file, metallic)
