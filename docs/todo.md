...
- [ ] **N.O.V.A. memory-flicker delivery** -- surface `nova_memory_flicker` as rare glitch-flavored gate/idle hints (same pacing discipline as Kaelen's hint plan: director-paced, never explained). The field is generated + stored + privacy-protected already; only the delivery beats are missing.
