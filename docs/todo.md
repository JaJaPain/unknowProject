...
- [ ] **Automatic story screenshots** -- PARTIAL 2026-07-04: `StoryScreenshots.gd` capture core done (frame_post_draw-timed grab, `<campaign>/screenshots/<unix>_<tag>.png`, 200-shot cap, headless-safe — note: root Window IS the viewport, `root.get_viewport()` returns null). Triggers live: campaign start (bible seed), chapter advance, story-hook resolution (skipped when chapter advance fires its own). STILL TODO: first jump to a new system, kill-cinematic frame, boss kill, first dock at a new station — those trigger sites live in GameRoot/CombatManager, not StoryManager. PDF embedding comes with campaign closure.
