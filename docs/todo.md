...
- [ ] **Player ship: try opposite tilt** — `PLAYER_SHIP_TILT_DEG` in `PlayerShip.gd` is 0 (upright) now. Tried +18° (leaned wrong way). Worth trying **-18°** (lean the other direction) to show more exhaust/top in the behind-above camera. One-line change to evaluate.
