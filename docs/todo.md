...
- [ ] **Gate "Attack Hostile" on range, with hysteresis** -- the target-window attack action should be unavailable while the selected hostile is out of reach, rather than letting the player commit to an attack they cannot make.
  ...
  - **Cheap:** only the CURRENTLY TARGETED ship is ever checked, so this is one distance test per tick, not a scan over every hostile.
  - **State:** hysteresis means the button isn't a pure function of distance — it depends on whether it was already enabled. That's **a single bool on the targeting side**, not per-ship state, since there's only one target at a time. Clearing it when the target changes falls out naturally: selecting a new target is what writes it.
  ...
  - **Watch:** the button is currently only refreshed on target change, but distance varies continuously while flying, so it needs re-evaluating as range closes and opens. The 600m gap between thresholds means a normal update tick is frequent enough — no need to poll hard.
