...
2. **Fix `_update_kaelen_mood()` leak vector** (§5.3): mood from hint stage/act state, or guard + overlap-check the output. *Files:* `StoryManager.gd`. *Risk:* low. *Test:* mood never shares distinctive tokens with the angle.
3. **Retry-with-errors for the bible** + clear the requested-slot guard on failure. *Why:* one flaky sample shouldn't strand a campaign. *Files:* `LLMInterface.gd` (`request_campaign_bible_generation`), `GameRoot.gd`. *Risk:* low-medium (watch double-request races; `campaign_bible_generation_in_flight` already guards). *Test:* inject invalid-then-valid responses, assert second attempt commits. *Feel:* fewer "story pending" stalls at new game.
4. **Repair telemetry.** Record every fired repair into `GenerationDiagnostics`. *Files:* `NarrativeDirector.gd`. *Risk:* trivial. *Test:* known-drifted fixture fires expected repair names.
5. **Store `creative_lane` in the bible** and surface it in DevPanel. *Risk:* trivial.

### Near-term (next couple of weeks)

6. **Title/reveal similarity gate + banned-repeat enforcement** (§3.2) with retry. *Files:* `NarrativeDirector.gd`, `CampaignIdeaMemoryStore.gd` (`query_recent`). *Risk:* low (pure code). *Test:* fixtures with near-duplicate titles rejected; distinct pass. *Feel:* second campaign never opens with a déjà-vu title.
7. **Faction triad in the bible** + `faction_pressure` state + block. *Files:* `NarrativeDirector.gd` (prompt+schema), `StoryManager.gd`, `LLMInterface.gd`. *Risk:* medium (schema change → migration backfill). *Test:* seed test maps faction problems into pressure state. *Feel:* Zenith/Aurelia/Vanguard read as actors, not set dressing.
8. **Lane rotation against history + opening_type/mystery-shape/pressure-type rotation** (§3.1/3.5/3.6). *Files:* `NarrativeDirector.gd`, idea memory. *Risk:* low. *Test:* consecutive campaign sims never repeat lane. *Feel:* rerolls feel like different games.
...
12. **Kaelen hint plan + paced delivery + hint-style rotation** (§5). *Files:* `NarrativeDirector.gd`, `StoryManager.gd`, `LLMInterface.gd` (kaelen_line path). *Risk:* medium. *Test:* hints deliver in order; undelivered hints never in small-model prompts. *Feel:* Kaelen becomes the reason players start a second campaign.
13. **Player choice memory + choice-aware horizon prompts** (§2.6). *Risk:* medium. *Feel:* the appended story references what *you* did.
14. **Real regeneration-trigger evaluation** — compute the metrics, honor all triggers, not just `triggers[0]`. *Files:* `StoryManager.gd`. *Risk:* low-medium.
