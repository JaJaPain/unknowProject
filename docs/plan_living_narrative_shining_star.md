...
- **Early footprint, delayed clarity:** the player sees consequences, methods, or beneficiaries early, while the full identity, motive, and scope unfold through earned evidence.
- **Active pressure:** every chapter has at least one code-visible move, aftermath, countermeasure, or escalating constraint attributable to the opposing force; the story does not wait passively for the player to visit it.
- **Personal but not exploitative stakes:** the force can threaten what Nova, Kaelen, and the Captain value, but it must not use arbitrary suffering or repeated hostage danger as a substitute for conflict.
- **Intelligent opposition:** it has coherent goals, capabilities, limits, and costs. It can win local contests, adapt, bargain, misdirect, or make a tempting offer without becoming omniscient or mechanically unfair.
- **Distinct relationship pressure:** its actions should create different friction for Nova and Kaelen, exposing their values and causing meaningful disagreement or growth rather than making both react the same way.
- **Earned confrontation:** final identification, direct contact, and decisive confrontation require enough player-safe evidence and prior pressure that they feel inevitable in hindsight.
...
- [x] Add one opposing-force dossier placeholder per campaign to the chapter packet: current footprint, known/unknown identity fields, objectives, capabilities, limits, chapter move, local aftermath, evidence trail, and escalation tier.
  - 2026-07-28: Chapter packets now always carry a validated `opposing_force` dossier. It starts explicitly `unformed` with empty structured fields, preserving old packets and preventing premature antagonist canon; later campaign design may transition it to `active` only when a selected core tension supports that decision.
...
- [x] Require every chapter packet to select only eligible attachment beats; prevent duplicate “vulnerability reveal” beats, unearned intimacy, and emotional escalation before the prerequisite shared event exists.
  - 2026-07-28: The attachment ledger projects only each character's next unfinished beat to chapter planning. Packet parsing rejects unavailable beats and duplicate character selections; an empty selection remains valid for chapters that do not need an attachment moment. Selecting a beat never completes it—only the existing code-owned event ledger can do that.
- [x] Give players room to miss or decline a bonding opportunity without punitive relationship loss; alternate later opportunities must still let trust grow through different actions.
  - 2026-07-28: Packet-selected attachment opportunities are carried into the mission metadata. Declining one does not lower Kaelen/N.O.V.A. rapport or Kaelen's relationship score, and it never consumes the event-driven attachment beat; later broadly eligible actions can still advance it. Ordinary non-attachment contract declines retain their existing consequences.
...
- [x] Add public prompt projections that include only the current approved state, relevant known facts, relationship lens, and situation rule. Director-only motivations remain outside the projection.
  - 2026-07-28: `FixedCastSoulRegistry.prompt_block()` now carries a bounded, de-duplicated three-fact `player_knows` projection alongside the current code-owned state, rapport band, attachment callback, and situation rule. Kaelen turn-ins and N.O.V.A. line-bank generation consume this projection; hidden truths remain excluded.
- [ ] Build separate large candidate banks for Nova and Kaelen by character, emotional state, campaign phase, and interaction kind. Label every candidate with its inputs and source; raw candidates never ship directly.
  - **Reference-sampling contract:** curate a broad approved library (target: roughly 30 examples per character/interaction family as it matures), but provide only 2–4 situation-matched references to any one generation. The sampler must avoid recently heard references and must choose from different semantic-premise tags rather than handing the model several variants of the same joke.
  - **Uniqueness contract:** every approved example must have a distinct semantic premise, player value, sentence shape, and emotional color. A wording change alone is not a new example; near-identical examples are rejected during curation. Persist a `semantic_premise_tag` and permit at most one reference from each tag in a prompt.
...
- [ ] Deduplicate approved banks by semantic premise, not just exact string: retain a small number of meaningfully different examples per state/situation and remove near-identical emotional beats.
  - Runtime validation must reject copied reference runs and known near-duplicate phrase patterns; examples guide rhythm and specificity, never player-facing wording.
...
- [ ] Validate that a generated line cannot name an emotion/state the code has not selected, reveal unknown facts, skip required phase gates, or drift into the other fixed character's voice.
  - 2026-07-26: Added `FixedCastLineValidator` and a 20-scenario deterministic audit covering state/situation validity, secret tokens, curated-copy detection, repeat detection, Kaelen/N.O.V.A. address drift, banned tics, and situation grounding. Kaelen completion generation and N.O.V.A.'s generated arrival, gate, hull, and combat-bank delivery now run through it before player delivery; movement-only lines retain their existing semantic-event gate.
...
- [x] Generate small flat banks in 6–10 field batches during chapter/system preparation; validate each line independently.
  - 2026-07-13: `LLMInterface.request_nova_line_bank_batch()` asks the small model (`nova_line_bank` capability, 30s background timeout) for one line per flat `@@label` field, capped at 10 labels per batch; the low-bank refill worker dispatches an 8-field batch (five movement semantics + three arrivals) whenever a consumed bank runs low — the consumption-driven refill IS the preparation hook, re-armed on every consume below threshold. Every line validates independently — length bounds, speaker-prefix rejection, placeholder braces, label leaks, duplicate labels/texts — and rejected lines are dropped, never repaired. Lines carry per-category kinds into the bank (`FallbackLineBank` dict-entry support). Parser/validator are pure statics proven in `run_nova_line_bank_batch_tests.gd`; the template bank is the logged degraded floor (`nova_line_bank / template_refill_used`).
- [x] Prompt with N.O.V.A.’s fixed persona, campaign quirk, allowed known facts, current system tone, and a bounded recent-event summary.
  - 2026-07-13: `GameRoot._nova_line_bank_generation_context()` supplies exactly that set: `Nova.PERSONA` verbatim, `story_state.nova_quirk` (prompt caps it at coloring one line), the campaign bible's public `tone`, the current system's display name as the allowed fact, and the observer's `recent_actions` streak (prompt-bounded to 6 entries, facts to 5). No director-only fields are read. Prompt assembly in `LLMInterface._nova_line_bank_prompt()`.
...
- [x] Add relevance scoring: prefer a story/mission-aware line over a generic movement joke when a real beat just occurred.
  - 2026-07-13: Generated bank lines are the story/system-aware ones (prompted with tone, quirk, and mission facts); template lines are the generic jokes. `FallbackLineBank.consume()` gained a `prefer_generated` pass that picks an unused generated-source line of the requested kind before any template line, falling back to normal order when none remain. Nova's movement handler sets it when the event's safe context shows a live mission beat (`mission_beat` present and `route_deviation != no_mission`). Proven in `run_fallback_line_bank_tests.gd` (preference, default order unchanged, fallback when generated lines run out) with wiring tripwires in `run_nova_tests.gd`.
...
- [x] When the player initiates undocking from a station with repair services, inspect the ship's hull-health band before release. If the hull is yellow or red and the player has not repaired during this dock visit, keep the ship docked while N.O.V.A. objects: stern in yellow, openly rude/self-preserving in red. Present “Go to repairs” and “Undock anyway”; stay silent for green hull, stations without repairs, canceled prompts, and completed repairs.
  - 2026-07-22: `UIManager.undock_player()` checks the full-service station and this-visit repair flag before release. A qualifying warning opens a persistent docked choice, then either enters Maintenance or performs the requested undock; `Nova.get_unrepaired_undock_warning()` is silent for healthy, repaired, unavailable-service, or destroyed states. `run_nova_tests.gd` covers the health/service/repair gates.
- [x] Use an instant authored line path only; the warning must never start a model request. Provide at least 12 distinct yellow-band objections and 12 distinct red-band comebacks, with red lines substantially ruder and more self-preserving. Record fallback use in diagnostics.
...
- [x] Replace per-reply generation with a pre-generated flat exchange bundle: opener, 2–3 code-approved player intents/questions, paired NPC answers, and a natural close.
  ...
  - 2026-07-13 (later): Transport landed — `LLMInterface.request_lounge_exchange_bundle()` on the small model (`lounge_bundle` capability, 25s background timeout, 520-token budget for the five fields), reusing the proven flat-JSON transport with a per-call options override. Registered and tripwired.
  - 2026-07-13 (runtime): Consumption landed. Rendering the lounge fires background preparation per contact card (code-selected intents → bundle request → parse + relevance validation → cached per stable contact key, reset each dock). `_start_lounge_conversation` prefers a ready bundle: opener and intent buttons appear instantly, an intent press displays the prepared answer then the prepared close with zero model requests (tripwired), degraded slots are never offered, and `gap:` intents record their fact IDs into `learned_fact_ids` for the NPC memory write at completion. The live per-turn path remains the fallback while a bundle is still preparing or failed.
- [x] Start with one meaningful player reply per bundle. For warm/story-important contacts, expose “keep talking” only when a second bundle is already cached. Do not preserve three turns by reintroducing visible waits.
  - 2026-07-13: The bundle flow is exactly one meaningful reply: opener → player question → prepared answer → prepared close. Consuming a bundle immediately starts preparing a second one in the background; at answer time, warm contacts (warmth ≥ 2) get a “Keep talking” choice ONLY if that second bundle is already `ready` — no readiness, no button, never a visible wait, and the button re-enters the same instant bundle flow (tripwired against any live model request in `run_lounge_conversation_tests.gd`).
- [x] Select player intents from knowledge gaps, NPC relationship, current stake, and delivered rumors. Avoid generic friendly/pushback/odd options when a natural story question exists.
  - 2026-07-13: Added `LoungeIntentSelector.select_intents()` — pure and code-owned. Priority order: rumored knowledge gaps (the player can only ask about what they have actually heard; unknown/known facts never become questions), delivered rumors, the current mission stake, then a warm-contact callback. Generic friendly/pushback/odd filler only pads up to the 2-intent minimum and never displaces or joins a full story set. Output is `{id, text, source}` entries that feed `build_bundle_prompt` verbatim. Covered in `run_lounge_intent_selector_tests.gd`. Still open: the live wiring that assembles the context (ledger gaps, rumor log, warmth store) at cache time.
- [x] Validate that each answer responds to its paired player line and remains in character.
  - 2026-07-13: `LoungeConversation.validate_bundle_answers()` runs after `parse_bundle`: every answer must contain at least one anchor token from the phrase its question was built around (`LoungeIntentSelector.anchor_tokens()` derives them from the alias/summary/mission label; anchor-less generic small talk accepts any reply) and must not contain out-of-character meta markers. Failing answers degrade to `""` per slot; a bundle with zero relevant answers is rejected. Fixture coverage in `run_lounge_conversation_tests.gd`: relevant pass, topic drift degraded, meta degraded, all-drift rejected, anchor derivation.
- [x] Mark rumors/facts heard only when the relevant answer is displayed. Fix the current approach path that can mark a hook heard while merely building the opener prompt.
  - 2026-07-13: Fixed the approach-path bug: `_lounge_approach_instruction` no longer calls `record_lounge_rumor_heard` at prompt-build time — it stashes `pending_hook_id` on the conversation state, and `_on_lounge_turn_result` records it only after the parsed opener line is appended for display. A failed/parse-rejected turn wipes the conversation (including the pending id), so the hook stays available for the next approach instead of being silently burned. Tripwired in `run_lounge_conversation_tests.gd` (instruction body must not record; display path must). The bartender/welcome rumor paths already recorded at display time and are unchanged.
- [x] Persist conversation stance and fact IDs learned into the NPC’s structured memory; summarize later.
  - 2026-07-13: `CampaignNpcStateStore.record_lounge_conversation()` persists the stance of the player's last reply onto `relationship.last_player_stance` plus a bounded raw history (`lounge_exchanges` ≤ 8, `lounge_fact_ids` ≤ 24, invalid fact IDs skipped) — raw now, summarize later as planned. `_apply_lounge_completion` feeds it through the new `GameRoot.record_lounge_conversation_memory()` bridge, which maps non-npc-namespace contact keys (`lounge.agent.zenith` → `npc.lounge.agent.zenith`) so validation holds. `learned_fact_ids` on the conversation state is the hook the bundle display path will populate. Store behavior + reopen persistence proven in `run_campaign_npc_state_store_tests.gd`; wiring tripwired in `run_lounge_conversation_tests.gd`.
- [x] Replace display-name keys with stable NPC IDs for warmth, cold-contact state, and once-per-dock rewards.
  - 2026-07-13: Audit found most paths already stable: every lounge card type carries a stable `contact_key` (`npc.fixed.kaelen`, `lounge.bartender.<station>`, `lounge.agent.<faction>`, generated cards from `npc_id`), warmth storage is contact-key-first with legacy-name migration, and cold contacts key through `_lounge_contact_key`. Fixed the one raw-name fallback: `_apply_lounge_completion` (the once-per-dock completion reward) now falls back to `_stable_lounge_contact_key(npc_id, name)` instead of the raw display name. Also moved the agent lead's `record_lounge_rumor_heard` inside the 3s delayed display callback — undocking during the delay no longer burns an undelivered lead. Tripwired in `run_lounge_conversation_tests.gd`.
- [x] Keep refusal/disposition mechanics code-owned. The model writes the way refusal sounds, not whether it occurs or how much reputation changes.
  - 2026-07-13: Already true and now tripwired. `LoungeConversation.agent_disposition()` is pure and code-owned (refusal flag, completion/bail rep, lead chance — unit-tested); `_start_lounge_conversation` short-circuits a refusal BEFORE any model request; the model only ever receives `context_line` as flavor; both rep deltas pull from the disposition dict. `run_lounge_conversation_tests.gd` now fails if the refusal check moves after the model call or the rep deltas stop coming from the disposition. Note: the refusal line itself is currently a code template (instant feedback by design, like the drink confirmation) — a prepared refusal bank could phrase it later without touching the mechanics.
- [x] Cache likely contact bundles during flight-to-station/system arrival. The card should indicate unavailable/pending before selection rather than turn a click into `Listening...`.
  - 2026-07-13: Partial — bundles now prepare the moment the lounge RENDERS (before any click), so the practical click-wait is gone for every card the player didn’t click within the first seconds of opening the lounge; a click before readiness falls to the live per-turn path. Still open for the strict reading: prefetch during flight/arrival (needs pre-dock contact prediction through the narrative cache scheduler) and a visible pending/unavailable indicator on the card itself (needs `_add_lounge_contact_card` UI work).
  - 2026-07-22: Completed strict path — selecting or issuing any navigation command to a station begins preparation of its predictable bartender, agent, and resident-contact bundles immediately. The cache is scoped to that target, retained only if that exact station is reached, and cleared on undock. A slow writer/reviewer leaves the existing disabled `PREPARING CONVERSATION` card state visible rather than allowing a live generation call from a click. Writer and independent reviewer prompts now explicitly reject terminal-like, menu-like, generic, or stock-signoff exchanges so cached text still reads as a normal short conversation.
...
- [x] NPC answers are relevant to the selected player line in 50/50 automated fixtures and >= 95% of a reviewed real-model batch.
  - 2026-07-22: The deterministic half is now covered by a 50-case sweep: 25 unique anchored-topic answers must survive validation, while 25 unique topic-drift or out-of-character answers must be rejected.
  - 2026-07-26: Isolated live fire ran 20 serial writer/reviewer exchanges across stranger, neutral, warm, and hostile contacts. All 20 passed bundle parsing, answer relevance validation, and the independent reviewer (`logs/lounge_bundle_live_fire_{01,06,11,16}.json`). One truncated writer response was recovered by the new single background retry; player clicks remain cache-only. Human scan found repetitive opener motifs, which belongs to the Phase 10 uniqueness/tone work rather than this relevance gate.
- [x] No rumor is marked heard until its delivery field is shown.
  - 2026-07-22: Existing lounge tests tripwire both delayed paths: an approach hook is held in `pending_hook_id` during prompt construction and only recorded after its opener displays, while an agent lead records only inside its delayed display callback. A failed request or an undock before display leaves the rumor available.
...
| 2026-07-26 | Phase 4A curated runtime voice bank | Added the human-approved fixed-cast voice bank. Public-board completion now selects its context-gated Kaelen `public_board_turn_in` lines with campaign-persistent non-repetition; legacy per-offer board text remains a diagnosed fallback. Normal Kaelen turn-ins use reviewed lines as their immediate fallback and non-copying style guidance for grounded model output. A generation guard rejects copied five-word reference runs, invented follow-up orders, and fictional credit adjustments; rejected responses recover through reviewed Kaelen lines. N.O.V.A. now uses matching reviewed lines between prepared generated banks and stock fallback pools. Added `FixedCastLineValidator`: Kaelen completion output must pass its state/situation, secret-leak, voice-drift, copy/repeat, and grounding checks; a deterministic 20-scenario audit covers both characters. | Tests: `run_fixed_cast_voice_bank_tests.gd`, `run_fixed_cast_line_validator_tests.gd`, `run_kaelen_interaction_bundle_tests.gd`, `run_nova_tests.gd`, `run_mission_contract_tests.gd`, `parse_check.gd`, `run_kaelen_curated_style_live_fire.gd`. |
...
| 2026-07-22 | Phase 8C follow-up | Changed the repair objection from a post-release bark into a docked N.O.V.A. decision: “Go to repairs” opens Maintenance; “Undock anyway” releases the ship. The old tutorial dock-arrow latch now persists through the tractor sequence instead of resetting back one step. | Tests: `run_nova_tests.gd`, `run_intro_handhold_tests.gd`. Commit `0cd1330`. |
| 2026-07-22 | Phase 10 persona guard | Removed UI-style faction-reputation framing from the station mechanic's prompt. Faction state now supplies private dockside subtext only; generated lines reject scores, standings, and labels such as “Wary rep.” | Tests: `run_mechanic_dialogue_tests.gd`. |
| 2026-07-22 | Phase 8C | Added an authored, repair-aware N.O.V.A. undock warning for damaged hulls at full-service stations. Yellow and red bands use independent campaign-persisted round-robin cursors (12 lines each); repairs during the dock visit suppress the warning. | Tests: `run_nova_tests.gd`, `parse_check_scene_scripts.gd`. |
| 2026-07-22 | Phase 7 reliability | Serialized large-model Kaelen handoff-batch generation and deferred queued batches while campaign-bible priority is active, preventing three simultaneous agent batches from competing with campaign initialization. | `run_kaelen_interaction_bundle_tests.gd` loaded the changed code and passed the new scheduling assertion; it still fails an unrelated pre-existing heart-then-profit prompt-shape assertion. Follow-up: live startup/chapter-refresh regression with Ollama. |
