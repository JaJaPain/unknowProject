...
- [ ] 0 post-tutorial dialogue-option clicks start an LLM request in automated V2 flow tests.
  ...
  - 2026-07-13: Added an automated cached-agent-offer V2 flow fixture that records a player click, presents ready cached text, and fails through `assert_no_click_to_generate_reports()` if any generation starts after the click. Full gate remains open until the other post-tutorial interaction families have equivalent coverage.
...
- [ ] No stale bundle survives a relevant story/knowledge/relationship mutation.
- [ ] Model contention cannot reproduce the current ~40-second sequential quest path.
...
- [ ] Define Kaelen interaction kinds: `agent_handoff`, `offer_comment`, `acceptance_afterthought`, `objective_complete_pending_turn_in`, `turn_in_clean`, `turn_in_rough`, `turn_in_late`, `partial_delivery`, `abandon`, `decline`, `chapter_comment`, and `first_system_arrival`.
- [ ] Keep the existing handoff pool, but key/refill it by story revision + agent + system + relationship band. Do not draw a line generated for a prior chapter’s pressure.
- [ ] Make `request_kaelen_intro()` consume a valid pooled/cached line first and route any refill through the central scheduler.
- [ ] Include mission cause, stake, giver, player’s asked questions, accepted terms, relevant prior memory, and safe Kaelen mood/style in handoff/turn-in prompt packets.
- [ ] Exclude `kaelen_hidden_angle`, undelivered hints, `kaelen_never_reveal`, and raw director truths from every small-model Kaelen packet.
- [ ] Replace `cached_completion_line` / `cached_abandon_line` UI globals with mission-keyed, persisted bundle fields.
...
- [ ] Add speaker/secret leak/line uniqueness validation and field-level degraded composition.
- [ ] Store the delivered Kaelen line fingerprint and event memory after actual playback.
...
- [ ] Retire consumed lines for the campaign. Refill before a bank reaches two remaining lines.
...
| 2026-07-13 | Phase 6 exit gate | Added a cached-agent-offer V2 flow fixture that records click/text lifecycle evidence and asserts no click-to-generate report occurs on the ready-cache path. | Tests: `run_narrative_cache_scheduler_tests.gd`. |
