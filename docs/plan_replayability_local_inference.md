...
| Slice | State | Notes |
|---|---|---|
| P2-7 sensor-range site reveal | **DONE** | `scripts/domain/SiteRevealModel.gd` + `tests/domain/run_site_reveal_tests.gd`. Sites are hidden beyond sensor range (600/900/1200 by tier) and leak NO label while hidden, appear as an anonymous "Signal contact" that fades in over the final 50 units, and earn their real name only once scanned. Deviation from the plan, deliberate: a SCANNED site stays visible at any range -- hiding something the player already found is losing their notes, not mystery. An unknown sensor tier falls back to basic rather than zero, so a typo in a ship definition weakens sensors instead of blinding them. Mutation-checked (removing the hide branch fails 4 assertions). |
