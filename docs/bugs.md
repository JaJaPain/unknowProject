...
| Date | Bug | Fix |
|---|---|---|
| 2026-09-14 | Live fire: the director returned "id collection.xxx" as every selected id, rejecting 5/5 proposals | The prompt wrote each job as `- id <ID>. The faction needs...`, so the writer copied the label with the id. The model was correct; the prompt was ambiguous. Jobs are now written `- <ID> = ...` with the id as the first token, plus an explicit instruction to copy it exactly and add no prefix -- `CampaignDirectionContract.gd` |
| 2026-09-14 | Live fire: the director declared a dependency in EVERY proposal, for jobs with no prerequisite relationship | `_validated_links` only checked that the cited fact was SUPPLIED, which is far weaker than the required "already supported by a real dependency". Nothing in the packet expressed a prerequisite, so no link was ever supportable and the writer filled the gap with decoration (reusing one generic backlog fact). `build_packet` now takes code-derived `prerequisites` and a link must match a supplied edge exactly; the prompt states when none exist. Links dropped to zero -- `CampaignDirectionContract.gd` |
| 2026-09-14 | Live fire: the raw station id "station.hub" reached player-facing prose | `public_direction` is shown to the player, so an internal identifier in it is a defect the code can catch. `validate_proposal` now refuses `identifier_in_public_direction` for the known id prefixes, matching only when a word character follows the dot so prose ending "...reached the station." still passes -- `CampaignDirectionContract.gd` |
| 2026-09-14 | Live fire: two structurally different campaigns produced near-identical prose | The writer received bare opaque ids and fact ids with NO text, so it had nothing specific to say and produced the same generic backlog sentence for every world. The packet now carries each candidate PUBLIC cause text (faction name, goal, need_reason, triggering_event, obstacle) -- the same vocabulary its board posting already shows -- and the prompt rejects generic filler. Two seeds now produce recognisably different campaigns -- `CampaignDirectionContract.gd` |
| 2026-09-14 | The campaign director would burn its whole repair budget on a request that could never succeed | The direction schema requires a proposal to cite a supplied premise fact, so a campaign with no KNOWN public facts cannot produce a valid answer at all -- but the flow still called the writer twice and then recorded a writer failure. `StoryManager._open_campaign_direction` now refuses up front with `no_public_premise_facts`, and `maybe_author_campaign_direction_live` checks the same condition before starting -- `StoryManager.gd` |
| 2026-09-14 | Every courier and pickup delivery edge was silently withheld | `CollectionOpportunityCompiler` modelled "this station can supply the item" as "this station's STORE sells it" for all three verbs. That is only true for `purchase_delivery`: a courier origin issues its consignment at acceptance (`GlobalState.accept_special`), and a pickup collects from a named contact. So every document and crate edge reported `source_does_not_supply_item` against a mechanic it never needed. Each action now asks for what its own capability enforces, and a pickup without a contact is refused with the new `no_source_contact` -- a check the store-only rule never made -- `CollectionOpportunityCompiler.gd`, `CollectionContract.gd` |
| 2026-09-14 | A collection contract and its board posting could disagree about the recipient | The world capture handed the opportunity compiler raw NPC DISPLAY NAMES, while the public-board recipient binding uses canonical `npc.<name>` IDs, so a posting could not prove it addressed the contact its contract bound. `StoryManager.collection_recipient_id()` now mints the canonical ID at capture time and `collection_recipient_record()` resolves it back, and `CollectionPostingBuilder` refuses any posting whose recipient does not match its contract -- `StoryManager.gd`, `CollectionPostingBuilder.gd` |
| 2026-09-14 | The tutorial contract was entering the pressure ledger and desire progress | `MissionOutcome.build` read `tutorial` from `quest.intro_tutorial_contract`, a field NOTHING in the codebase sets, and which `MissionAdapter` does not carry onto the accepted mission either. So `StoryManager.stage_mission_outcome`'s `tutorial_excluded` branch never fired for the real starter contract. Found by the new campaign trace test. `MissionOutcome._is_tutorial_contract` now also matches the fixed shape QuestManager uses ("Clean and Easy" / KILL_SHIPS / reavers) -- `MissionOutcome.gd` |
| 2026-09-14 | The tutorial contract also counted toward discretionary family pacing | Same root cause: the accepted mission state carries no tutorial flag to check. `LocalPressureDirector.family_for_mission` now checks `intro_tutorial_contract`, `is_intro_tutorial`, `story_required` AND the starter contract's fixed shape, and returns "" for all of them -- `LocalPressureDirector.gd` |
| 2026-09-14 | Abandoning a courier stranded its crate in the hold | `abandon_quest` never called `_cleanup_mission`, so the capability cleanup that expiry runs (which clears matching special cargo) was skipped entirely on the abandon path. Cleanup now runs INSIDE the terminal transaction, so a failed checkpoint gives the crate back -- `QuestManager.gd` |
| 2026-09-14 | A rolled-back settlement destroyed live investigation scan state | `abandon_quest` called `_investigation_runtime.reset()` before its snapshot, and `complete_quest` after its signals, so a failed checkpoint lost the scan without restoring it. The reset now happens only on the committed branch -- `QuestManager.gd` |
| 2026-09-14 | An unbuildable mission outcome reported itself as durably saved | `_settle_terminal` returned `durable: true` when `MissionOutcome.build` failed, which claimed persistence for a record that was never written. A mission carrying typed story bindings now fails closed; a genuinely legacy one terminates as `compatibility: true, durable: false` -- `QuestManager.gd` |
