...
CREATIVE_LANES = [
    {"name": "criminal economy", "guidance": "center smuggling, forged manifests, protection rackets, stolen cargo, and who profits when legal trade breaks."},
    {"name": "corporate espionage", "guidance": "center deniable sabotage, leaked patents, dirty audits, black-budget security, and contracts that hide ownership."},
    {"name": "infrastructure collapse", "guidance": "center failing stations, broken relays, unsafe gates, repair scarcity, and factions blaming each other for neglect."},
    {"name": "political succession", "guidance": "center leadership disputes, emergency powers, contested permits, quiet coups, and brokers selling access."},
    {"name": "salvage rights", "guidance": "center wreck claims, disputed manifests, dead ships, insurance fraud, and evidence hidden in recovered parts."},
    {"name": "debt and privatization", "guidance": "center repossession, company towns, privatized security, predatory loans, and survival under owned infrastructure."},
    {"name": "ecological or industrial hazard", "guidance": "center toxic ore, failing life support, unsafe extraction, poisoned habitats, and coverups disguised as accidents."},
]
...
def lane_for_seed(seed):
    ...
    for i, ch in enumerate(seed):
        acc += ord(ch) * (i + 1)
    return CREATIVE_LANES[abs(acc) % len(CREATIVE_LANES)]

def build_prompt(seed):
    lane = lane_for_seed(seed)
    return "\n".join([
        ...
        "Creative lane for this campaign: %s." % lane["name"],
        "Lane guidance: %s" % lane["guidance"],
        "",
        "Anti-motif guidance:",
...
def call_ollama(prompt, seed_int):
    body = {
        ...
        "options": {"temperature": 0.95, "num_predict": 900, "seed": seed_int},
    }
...
def kaelen_violates(rule):
    lower = rule.lower()
    if any(k in lower for k in ["broker", "fixer", "contract"]):
        return False
    return True

if __name__ == "__main__":
    stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = os.path.join(r"C:\CodingProjects\SpaceGame\OllamaTestStories", f"{stamp}_claude_current_prod_check")
    os.makedirs(out_dir, exist_ok=True)
    seeds = ["campaign-a1", "campaign-b7", "campaign-c3", "campaign-d9", "campaign-e2", "campaign-f5"]
    results = []
    for i, seed in enumerate(seeds, 1):
        prompt, lane_name = build_prompt(seed)
        result, elapsed = call_ollama(prompt, i * 101)
        raw = result.get("response", "")
        fname = os.path.join(out_dir, f"cand_{i:02d}_{lane_name.replace(' ', '_')}.json")
        with open(fname, "w", encoding="utf-8") as f:
            f.write(raw)
        try:
            data = json.loads(raw)
            title = data.get("campaign_title", "")
            reveal = data.get("long_term_reveal", "")
            kaelen_rule = data.get("kaelen_rule", "")
            violates = kaelen_violates(kaelen_rule)
            print(f"[{i}] lane={lane_name} elapsed={elapsed:.1f}s title={title!r}")
            print(f"     reveal={reveal!r}")
            print(f"     kaelen_rule={kaelen_rule!r} VIOLATES_PUBLIC_MASK={violates}")
        except Exception as e:
            print(f"[{i}] lane={lane_name} PARSE FAILED: {e}")
        results.append({"seed": seed, "lane": lane_name, "elapsed": elapsed})
    print(f"\nSaved to {out_dir}")
